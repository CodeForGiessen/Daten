...
function processLakeProfileQ(lakeprofilelink,lake){
  ...
  .then(function(html){
    ...
    lakebind.extracurricular_activity = table.eq(8).children().eq(1).text().split(',').map(function(s){
      return s.trim();
    });
...
function processLakeMeasurementsQ(measurementurls, result){
  ...
  if(typeof(measurementurl) !== 'undefined'){
    ...
    .then(function(html){
      var $ = cheerio.load(html);
      $('table.messdaten tr:not(:first-child)').filter(function(){
        var row = $(this);
        ...
        var measurement = {
          'date': date,
          'water_itemperature': waterTemperature,
